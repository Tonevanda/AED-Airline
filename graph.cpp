...
void Graph::getShortestFilteredPathFromList(list<int> start,const string& end,const set<string>& permittedAirlines){
    clearData();
    for(auto v:start){
        int finalpos=INT_MAX;
        queue<int> q; // queue of unvisited nodes
...
unordered_map<string,vector<Airport>> Graph::getCityAirports(){
    return cityAirports;
}